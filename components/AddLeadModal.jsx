  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start z-50 overflow-y-auto py-10">
      <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-4xl">
        {/* UPDATE: Styling for the header */}
        <h2 className="text-2xl font-bold text-slate-800 mb-6">Add New Contact</h2>
        <form onSubmit={handleSubmit}>
          <FormSection title="Lead Information">
            ...
            <Select label="Lead Source" name="lead_source" value={formData.lead_source} onChange={handleChange}>
            ...
            <Select label="Price List" name="price_tier" value={formData.price_tier} onChange={handleChange}>
              <option value="Public/Direct">Public/Direct</option>
              <option value="Partner/Vendor">Partner/Vendor</option>
            </Select>
          </FormSection>
          <FormSection title="Event Details">
            <Input label="Event Date" type="date" name="event_date" value={formData.event_date} onChange={handleChange} required />
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {servicesList.map(service => (
                <label key={service} className="flex items-center space-x-2">
                  <input type="checkbox" value={service} onChange={handleCheckboxChange} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
          ...
          <div className="mt-8 flex justify-end space-x-4">
            ...
            {/* UPDATE: Styling for the save button */}
