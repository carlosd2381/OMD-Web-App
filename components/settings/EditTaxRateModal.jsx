...
function EditTaxRateModal({ isOpen, onClose, onRateUpdated, editingRate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from('tax_rates').update(updateData).eq('id', id);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-md">
        <h2 className="text-2xl font-bold text-[var(--color-brand-brown)] mb-6">Edit Tax Rate</h2>
