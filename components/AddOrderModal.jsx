...
function AddOrderModal({ isOpen, onClose, onOrderAdded, clientData }) {
  // UPDATE: Add state for menuItems
  ...
  const [soldAtDate, setSoldAtDate] = useState(new Date().toISOString().split('T')[0]);
  ...
  useEffect(() => {
    if (isOpen) {
      setOrderName('New Order');
      setSelectedItems([]);
      setTaxRate(null);
      setError(null);

      // UPDATE: Fetch both menu items and tax rates
  ...
  const handleAddItemToQuote = (item) => {
    setSelectedItems(prevItems => {
      ...
      if (existingItem) {
        return prevItems.map(i => i.menu_item_id === item.id ? { ...i, quantity: i.quantity + 1 } : i);
      } else {
        return [...prevItems, { 
  ...
  const handleQuantityChange = (itemId, newQuantity) => {
    ...
    if (qty > 0) {
      setSelectedItems(prevItems => prevItems.map(item => item.menu_item_id === itemId ? { ...item, quantity: qty } : item));
    }
  };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
        <div className="bg-gray-100 rounded-lg shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col">
          <div className="flex justify-between items-center p-4 border-b bg-white rounded-t-lg flex-shrink-0">
            <h2 className="text-2xl font-bold text-gray-800">New Order for {clientData.full_name}</h2>
            <div className="flex space-x-2">
              ...
              <button onClick={handleSaveOrder} className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700" disabled={isSubmitting}>
            ...
            <div className="flex-1 p-6 overflow-y-auto">
              <div className="bg-white p-6 rounded-lg shadow-lg">
                <FormSection title="Details">
                  <Input label="Name" value={orderName} onChange={(e) => setOrderName(e.target.value)} />
                  <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Input label="Sold At" type="date" value={soldAtDate} onChange={(e) => setSoldAtDate(e.target.value)} />
                ...
                <FormSection title="Order Configuration">
                  ...
                  <div className="mt-4 border rounded-lg p-4 min-h-[150px]">
                    {selectedItems.length > 0 ? (
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b"><th className="text-left font-semibold p-2">Item</th><th className="text-left font-semibold p-2 w-24">Qty</th><th className="text-right font-semibold p-2">Price</th><th className="text-right font-semibold p-2">Total</th><th className="p-2"></th></tr>
                        </thead>
                        <tbody>
                          {selectedItems.map(item => (
                            <tr key={item.menu_item_id} className="border-b">
                              <td className="p-2">{item.name}</td>
                              <td className="p-2"><Input type="number" value={item.quantity} onChange={(e) => handleQuantityChange(item.menu_item_id, e.target.value)} className="w-20 text-center" min="1" /></td>
                              ...
                              <td className="p-2 text-center"><button onClick={() => handleRemoveItem(item.menu_item_id)} className="text-red-500 hover:text-red-700 font-bold text-lg">×</button></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            <aside className="w-72 bg-white p-6 border-l flex flex-col justify-between flex-shrink-0">
              <div>
                ...
                <div className="space-y-2">
                  <ActionButton>Book & Email</ActionButton>
